...
## Goals
- G1: 输入攻略链接（小红书/抖音/微信公众号/穷游等图文/视频）或截图，30 秒内自动提取地点、天数、顺序，生成按天排列的可视化行程。
- G2: 行程手册视觉质量接近用户现有 `index.html`（暖色调编辑风格）与参考海报（图文混排长图）的水平，并保留 8 大模块结构作为输出格式。
- G3: 提供可交互的管理工具：行前打包清单、预算追踪、每日行程编辑，且状态本地持久化。
- G4: 配置一次大模型 API 即可运行；前端可独立工作（手填目的地模式），启用 Python 后端 + DSH 时获得"链接解析 + 排线 + 协作 + 动态优化"完整能力。
- G5: 生成结果可导出为静态 HTML 或长图，便于分享/打印。
- G6: 通过 Scrapling 爬虫抓取真实旅游信息（景点描述、门票、价格、坐标、图片），降低对 LLM 长文本生成的依赖，节省 token 并提升内容真实性。
- G7: LLM 在流水线中承担四类职责：①驱动爬虫与解析 ②抽取结构化信息 ③校对验证 ④最终效果优化。
- G8: 由 DeepSeek Harness（Cordis 全插件框架）承担编排，把"链接解析/抽取/排线/优化/缓存/协作/监测"封装为可热替换的标准插件，LLM 在框架内通过工具调用与子 Agent 委派"守规矩办事"，避免硬编码 Agent 主循环。
- G9: 通过 DSH Trajectory 日志与 Creator 模式为后续自进化（插件热替换、轨迹回放、策略迭代）留出接口；v1 仅做可观测与可回放，不强制上线自进化。
- G10: 地理空间智能排线：基于高德/百度地图 API 计算景点间距离与交通时间，自动按"同区域集中 + 最少折返"原则排到对应日期，避免"同一天跑三个不相邻区域"。
- G11: 多人实时协作编辑：行程可拖拽排序、多人同步编辑，适合家庭/朋友组队出行，编辑延迟 ≤ 1 秒。
- G12: 动态优化引擎（"酷炫"主动进化）：DSH Agent 持续监测目的地天气、当地节庆/季节特色、与"最新同城好玩"的爬取结果；当检测到更适合的元素时，自动向行程注入新卡片（如某音乐节/市集）或替换原有元素（如下雨天把户外爬山换成博物馆），所有变更通过 DSH Web UI 驾驶舱以可撤销的方式呈现给用户，与原方案对比展示。
- G13: 完全免费、无广告、无预订商业倾向，与 OTA 平台互补而非竞争。

## Non-Goals
- 不做用户登录/注册系统（v1 用分享链接 + 编辑 token）；不做云端用户账号体系。
- 不做真实机票/酒店/门票预订与支付（与 OTA 平台互补定位）。
- 不做社区内容创作、UGC 信息流、关注关系。
- 不做实时导航、离线地图下载。
- 不做移动端原生 App（仅响应式 Web）。
- 不抓取需登录或付费墙内容（小红书登录后内容、公众号付费阅读等不抓）。
...
### 实时动态优化（"酷炫"主动进化，DSH Web UI 驾驶舱）
- **FR-27**: 监测层（由 `tg-monitor` 插件实现）：周期性拉取目的地天气（Open-Meteo 免费 API 默认；和风天气/QWeather 可选配置）、当地节庆日历、景点官方开放状态；周期性触发 `tg-scraper` 按"目的地 + 今日/明日"维度爬取同城热榜（演出、市集、展览、限时活动、新开业店铺），结果带 `source_url` 与时间戳；监测频率可配（默认每小时 1 次，出行前 24 小时自动加密到每 30 分钟）。
- **FR-28**: 当地特色感知库：维护一份季节/地域特色素材库（如"呼伦贝尔 7 月那达慕""济南 8 月荷花节""哈尔滨 1 月冰雪大世界"），由 LLM + 规则混合生成并经 `tg-validator` 校对后入库；每次生成行程时按出行日期匹配并注入对应特色元素到对应日期卡片（按用户偏好白名单过滤，不打扰）。
- **FR-29**: 动态元素注入/替换引擎（核心）：`tg-optimizer` 在监测数据触发阈值时产出结构化 diff（`ADD` 新卡片 / `SWAP` 替换卡片 / `MOVE` 调整顺序），交 `tg-validator` 二次校对后通过 WebSocket 推送到前端；前端按 diff 原子更新 DOM，并展示"为什么这次变更"的可读说明（如"明日济南有雷阵雨，已将千佛山 → 山东省博物馆，预计游玩时长不变"）；变更前后对比卡片以高亮动画呈现。
- **FR-30**: DSH Web UI 作为人机协同驾驶舱：后端启动 `dsh web`（默认 `http://127.0.0.1:3080`），用户可在驾驶舱看到 Agent 的思考轨迹、即将推送的动态变更建议列表、阈值配置面板；用户可在 Web UI 内"采纳/驳回/编辑"某条建议，操作通过 WebSocket 双向同步到前端；所有操作进入 Trajectory 日志便于复盘。
- **FR-31**: 用户偏好与打扰阈值：设置面板提供"动态更新"总开关 + 频率上限（默认出行前 ≥24h 时 ≤ 每 2 小时 1 次；出行前 24h 放宽到每小时 1 次；行程进行中按当日实际状况触发，无频率硬上限但单日 ≤ 5 条）+ 元素类型白名单（如只接受"美食/活动/展览"类 ADD，不接受替换住宿与跨日移动）+ 自动应用模式（默认"建议模式"需用户点确认，可选"自动应用模式"由 Agent 直接应用并允许撤销）。
- **FR-32**: 可撤销与版本快照：每次动态变更生成 plan 版本快照（`version_id` + 时间戳 + 触发原因 + diff），用户可一键回滚到任意历史版本；行程手册顶部展示"动态更新 N 次"小徽章，点击查看 changelog（时间/触发原因/变更类型/采纳状态）。
- **FR-33**: 协同冲突解决：多人协作编辑与动态优化同时发生时，以"用户手动编辑优先"为原则——若用户在最近 5 分钟内手动编辑过某卡片，动态优化跳过该卡片仅推送提示；冲突时以 Trajectory 中最新用户操作时间戳为准。
...
- **NFR-9（可观测性）**: 后端模式下，每次生成返回 `session_id`，前端可拉取 Trajectory 轨迹并在调试视图中按时间线展示；所有 LLM/Scrapling/subagent/动态优化调用进入 append-only 日志，便于复盘与回放。
- **NFR-10（演进友好）**: 所有 `tg-*` 插件遵循 Cordis 插件契约，支持热重载与配置级组合；v1 不强制上线自进化，但所有可热替换点必须可被 Creator 模式实验与 `/plugin/reload` 接口验证。
- **NFR-11（实时性）**: 动态优化推送走 WebSocket，从后端 Agent 决策到前端 DOM 更新端到端 ≤ 3 秒；DSH Web UI 驾驶舱内的"采纳/驳回"操作反向同步到前端 ≤ 1 秒。
- **NFR-12（驾驶舱可访问性）**: DSH Web UI 驾驶舱在 `127.0.0.1:3080` 仅本机访问，不暴露到公网；默认无需登录，但 README 注明若需远程访问应叠加反代 + 鉴权。
...
### AC-14: 动态元素注入/替换可见且可撤销
- **Type**: `rule`
- **Given**: 已生成行程 + 后端 DSH + `tg-monitor` 在线 + 模拟触发条件（如出行日某地天气为大雨且同城有室内展览事件）
- **When**: `tg-optimizer` 检测到触发阈值并产出 diff
- **Then**: 前端通过 WebSocket 收到 diff，对应日期卡片以高亮动画"换出"户外景点、换入"室内展览"；变更说明文字可见（如"明日济南有雷阵雨，已将千佛山 → 山东省博物馆"）；点击"撤销"可一键回滚到上一版本；顶部"动态更新 N 次"徽章计数 +1
- **Pass Condition**: 变更前后 DOM diff 可见、`source_url` 与说明文案非空、撤销按钮可还原原方案、徽章计数正确
- **Evidence**: 变更前后截图 + 撤销录屏 + WebSocket 消息日志

### AC-15: DSH Web UI 驾驶舱可观测与双向控制
- **Type**: `rule`
- **Given**: 后端运行中 + `dsh web` 已启动（默认 `http://127.0.0.1:3080`）
- **When**: 用户在浏览器打开驾驶舱 URL
- **Then**: 看到当前会话的思考轨迹、即将推送的动态变更建议列表、阈值配置面板三类视图；可在 Web UI 内点"采纳/驳回/编辑"某条建议
- **Pass Condition**: 驾驶舱可见上述三类视图；采纳后前端行程在 ≤ 1 秒内同步更新；驳回则该建议不推送至前端
- **Evidence**: DSH Web UI 截图 + 采纳后前端同步更新截图 + WebSocket 消息日志
...
- [x] ~~**Q9 内容来源**~~：自动爬为主推，用户粘贴（链接/文本/截图）为兜底与微调路径（FR-14）。
- [x] ~~**Q10 动态优化交互**~~：默认"建议模式"（用户确认后应用），可选"自动应用模式"（Agent 直接应用允许撤销）；冲突时用户手动编辑优先（FR-31、FR-33）。
...
- [ ] **Q11 天气 API**：默认 Open-Meteo 免费（免 key），是否提供和风天气/QWeather 作为可选配置？倾向默认 Open-Meteo，和风作为可选。
- [ ] **Q12 多人协作后端**：v1 多人协作编辑走自建 WebSocket 中转还是接 Y.js/Automerge CRDT？倾向 v1 走轻量自建 WebSocket 中转（依托 FastAPI），CRDT 留作规模化时升级。
